"""Reviewer for Designer output. Updates same planning_artifacts row (reviewer_pass, reviewer_notes)."""
...
def run_design_reviewer(limit: int = 5) -> None:
    with open("config/design_reviewer_checklist.yaml") as f:
        checklist = yaml.safe_load(f)
    checks_text = _checks_text(checklist)

    # Any designer artifact not yet passed (revision_count not used here; designer enforces max_revisions)
    all_rows = (
    ...
    if not pending:
        print("[design_reviewer] no designer artifacts pending review (all passed or none found)")
        return

    for row in pending:
        idea_id = row["judged_idea_id"]
        designer_output = json.dumps(row["payload"], indent=2)
        prompt = (
            f"CHECKLIST:\n{checks_text}\n\n"
            f"Designer output for idea {idea_id}:\n\n{designer_output}\n\n"
        ...
        try:
            ...
            print(f"[design_reviewer] updated planning_artifacts for {idea_id}")
        except Exception as e:
            print(f"[design_reviewer] failed to update review for {idea_id}: {e}")


def run_design_reviewer_for_idea(idea_id: str) -> None:
    res = (
        supabase.table("planning_artifacts")
    ...
    if not res.data:
        print(f"[design_reviewer] skip {idea_id}: no designer artifact in planning_artifacts")
        return
    with open("config/design_reviewer_checklist.yaml") as f:
        checklist = yaml.safe_load(f)
    checks_text = _checks_text(checklist)
