"""Architecture reviewer: validates Architect output against a checklist.

Updates the same planning_artifacts row (reviewer_pass, reviewer_notes, revision_count)
— no new row, like analyzed_ideas + analyst reviewer.
"""
...
def run_arch_reviewer(limit: int = 5) -> None:
    """Review architect rows in planning_artifacts; update same row with reviewer_pass, reviewer_notes."""
    with open("config/arch_reviewer_checklist.yaml") as f:
        checklist = yaml.safe_load(f)
    checks_text = _checks_text(checklist)

    # Any architect artifact not yet passed (revision_count not used here; architect enforces max_revisions)
    all_arch = (
    ...
    pending = [
        r for r in (all_arch or [])
        if r.get("reviewer_pass") is not True
    ][:limit]
    if not pending:
        print("[arch_reviewer] no architect artifacts pending review (all passed or none found)")
        return

    for row in pending[:limit]:
        idea_id = row["judged_idea_id"]
        arch_output = json.dumps(row["payload"], indent=2)
        prompt = (
            f"CHECKLIST:\n{checks_text}\n\n"
            f"Architecture output for idea {idea_id}:\n\n{arch_output}\n\n"
            "Assess the architecture against the checklist above. Output only PASS, REVISE, or ESCALATE with failed_checks and revision_notes as per the rules."
        )
        response = call_agent(
            "arch_reviewer",
            REVIEWER_SYSTEM_INSTRUCTIONS,
            prompt,
            schema=ARCH_REVIEWER_SCHEMA,
            effort="low",
        )
        ...
        try:
            ...
            print(f"[arch_reviewer] updated planning_artifacts for {idea_id}")
        except Exception as e:
            print(f"[arch_reviewer] failed to update review for {idea_id}: {e}")


def run_arch_reviewer_for_idea(idea_id: str) -> None:
    """Review the architect row for one idea; update same row (reviewer_pass, reviewer_notes)."""
    res = (
        supabase.table("planning_artifacts")
    ...
    if not res.data:
        print(f"[arch_reviewer] skip {idea_id}: no architect artifact in planning_artifacts")
        return
    with open("config/arch_reviewer_checklist.yaml") as f:
        checklist = yaml.safe_load(f)
    checks_text = _checks_text(checklist)
